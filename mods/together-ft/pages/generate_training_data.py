...
def init_weave_client(default_project="example"):
    ...
    with st.sidebar:
        ...
        try:
            ...
            print(f"Error connecting to Weave:\n{traceback.format_exc()}")
            st.error(f"Error connecting to Weave:\n{e}")
            st.stop()
...
def generate_input_output_pairs(task_description: str, num_pairs: int = 10, temperature: float = 1.0, last_pairs: list[InputOutputPair] = []) -> InputOutputPairList:
    """
    ...
    try:
        response = openai.beta.chat.completions.parse(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are a helpful assistant that generates input-output pairs in JSON format."},
    ...
    if st.button("Generate"):
        ...
        for i in range(num_calls):
            ...
            temperature = temp_min + \
                (temp_max - temp_min) * \
                (i / (num_calls - 1)) if num_calls > 1 else temp_min
            inp_out_pairs = generate_input_output_pairs(
                task_description, num_examples_generated_per_call, temperature=temperature, last_pairs=all_input_output_pairs[-10:])

            # Create batch-specific list
            batch_pairs = [{"input": pair.inp, "output": pair.out}
                           for pair in inp_out_pairs.pairs]
        ...
        training_data_dataset = Dataset(
            name=dataset_name, description=f"Dataset generated from the following task: {task_description}", rows=all_input_output_pairs)
        published_dataset = weave.publish(training_data_dataset)

        training_data_dataset_metadata = {"dataset_name": dataset_name,
