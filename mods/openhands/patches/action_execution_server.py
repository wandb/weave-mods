...
if __name__ == "__main__":
    ...
    async def download_file(path: str):
        ...
        logger.debug(f"Starting download for {path}")
        try:
            ...
            # Create zip file first to know the total size
            ...
            try:
                ...
                with ZipFile(temp_zip.name, "w") as zipf:
                    for root, _, files in os.walk(path):
                        for file in files:
                            # TODO: just use .gitignore
                            if file.startswith(".venv") or ".mypy_cache" in file:
                                continue
                ...
                logger.debug(
                    f"Zip creation took {time.time() - zip_start:.2f}s for {file_count} files ({total_size/1024/1024:.2f}MB), zip size: {zip_size/1024/1024:.2f}MB"
                )
                ...
                logger.debug(
                    f"Total download preparation took {time.time() - start_time:.2f}s"
                )

                headers = {
                    "Content-Disposition": f"attachment; filename={os.path.basename(path)}.zip",
                    "Content-Length": str(zip_size),  # Set exact content length
            ...
            logger.error(f"Download failed after {time.time() - start_time:.2f}s: {e}")
            raise HTTPException(status_code=500, detail=str(e))
